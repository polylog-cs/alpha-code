...
class Solution2(Scene):
    def construct(self):        
        ...
        self.play(starting_tree.animate().shift(-1.5*H + 5*sh))
        ...
        # other_sols = Tex("There are also other solutions. ", color = GRAY).scale(0.35).shift(3.5*DOWN + 5.5*RIGHT)
        # self.play(FadeIn(other_sols))
        # self.wait()
        # self.play(FadeOut(other_sols))
        # self.wait()
...
class AlphaCode(Scene):
    def construct(self):
        code_left = ImageMobject("img/code1.png").scale_to_fit_height(8).align_to(Dot(radius=0).move_to(7.1 * LEFT), LEFT)
        code_right = ImageMobject("img/code2.png").scale_to_fit_height(8).next_to(code_left, RIGHT, buff = 0).shift(2*LEFT)
        ...
        line_height = 0.3
        rec1 = Rectangle(color = RED, height = line_height, width = 1.5).move_to(0.6*RIGHT + 0.05*DOWN)
        rec2 = Rectangle(color = RED, height = 3.6*  line_height, width = 7).move_to(0.6*RIGHT + 0.9*DOWN).align_to(rec1, LEFT)
        rec3 = Rectangle(color = RED, height = 6.7 * line_height, width = 6).move_to(4*LEFT + 0.58*UP)
...
class Dogs(Scene):
    def construct(self):
        random.seed(8)
        dogs = [
            ImageMobject("img/dogs/p{}.jpg".format(i)).scale_to_fit_width(2).move_to(
                2*UP
            ) for i in range(1, 7)
        ]
        muffins = [
            ImageMobject("img/muffins/p{}.jpg".format(i)).scale_to_fit_width(2).move_to(
                2*UP
            ) for i in range(1, 9)
        ]

        dogs_position = 2*DOWN + 3*LEFT
        for dog in dogs:
            dog.generate_target()
            dog.target.move_to(dogs_position)
        muffin_position = 2*DOWN + 3*RIGHT
        for muffin in muffins:
            muffin.generate_target()
            muffin.target.move_to(muffin_position)
                   
        dog_title = Tex("Dog", color = GRAY).next_to(dogs[0].target, DOWN)
        muffin_title = Tex("Muffin", color = GRAY).next_to(muffins[0].target, DOWN)
        ...
        for i in range(len(pics)):
            pics[i].set_z_index(-i)
            pics[i].target.set_z_index(i)
            self.add(pics[i])
    
        for pic in pics:
            pic.set_z_index(i)
            self.play(MoveToTarget(pic), run_time = 0.3)
...
#         s = [
#             "Big thanks to",
#             "-- 3blue1brown and Manim Community,",
#             "-- Filip Hlásek, Matěj Konečný, Hanka Rozhoňová, Tom Sláma, ",
